<?php
  if(isset($_GET['searchiddoimovel'])) {
    ...
    $omsselect = "SELECT * from files WHERE iddoimovel=$iddoimovel AND tipo='5' AND imobiliaria_creci=$lgnimobiliaria_creci";
    try {
  ...
  ?>
          <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-left-info shadow h-100 py-2">
              <form method="POST"><label class="btn-trash" for="contratodeadministracao"><i class="fas fa-times"></i></label><input type="submit" id="contratodeadministracao" name="contratodeadministracao<?php echo $omsid; ?>" hidden></form>
  ...
  <?php
          if(isset($_POST['contratodeadministracao'.$omsid])) {
            unlink($omsfile);
